...
const getRazorpay = async() => {
    ...
    try {
        const Razorpay = (await
            import ('razorpay')).default;
...
router.post('/razorpay/verify', authenticate, authorize('customer'), async(req, res) => {
    try {
        ...
        // Mark order paid
        const order = await Order.findOneAndUpdate({ paymentTransactionId: razorpay_order_id }, {
            paymentStatus: 'paid',
            status: 'confirmed',
            $push: {
                statusHistory: {
    ...
    async(req, res) => {
        try {
            ...
            if (event.event === 'payment.captured') {
                ...
                await Order.findOneAndUpdate({ paymentTransactionId: order_id, paymentStatus: { $ne: 'paid' } }, {
                    paymentStatus: 'paid',
                    status: 'confirmed',
